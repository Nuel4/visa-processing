<%-- 
    Document   : AffiConect.jsp
    Created on : 16 Feb, 2019, 1:20:26 PM
    Author     : Admin
--%>
...
<td><a href="index.html"> <i>Education & Culture</i></a></td></tr>
        ...
        <%
           try
           {
               na = Integer.parseInt((String) session.getAttribute("log"));
               
                              Class.forName("com.mysql.jdbc.Driver");
                              Connection n= DriverManager.getConnection("jdbc:mysql://localhost:3306/columbiacollege", "root", "root");
                              Statement me=n.createStatement();
                         ResultSet es  = me.executeQuery("select *from enrolldb where regno = "+na+"");
                         if(es.next())
                         {
                             st=es.getString(28);
                                 
                         
                             if(st.equals("DECLINE"))
                             {
                               
                               out.print("<br> Decline! your affidavit is rejected");
                             }
                             else
                               {
                                 if(st.equals("processing"))
                                 {
                                 out.print("<br>your affidavit is under evaluation.please check back later!");
                                }
                               
                             else
                                  {
                                    response.sendRedirect("AffiSubmit.jsp");
                                     }
                               }
                         }
