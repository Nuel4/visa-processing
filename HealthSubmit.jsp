<%-- 
    Document   : HealthSubmit.jsp
    Created on : 16 Feb, 2019, 10:08:22 PM
    Author     : Admin
--%>
...
.mm{margin-top: 40px;font-family: sans-serif;font-size: 25px; margin-bottom: 70px;color: brown; padding-top: 25px; display: ruby-base;}
...
<td><a href="index.html"> <i>Education & Culture</i></a></td></tr>
        ...
        <center> <h1 class="mm">HEALTH FORM CONFIRMATION!</h1></center>
      <%!
            int pd;
            int f=0;
        %>
        <%
            ...
            String reg=request.getParameter("hlt");
             String hos=request.getParameter("hpt");
              String locat=request.getParameter("loc");
               String doc=request.getParameter("dr");
                String stau=request.getParameter("sta");
                 String pla=request.getParameter("mon");
                  String auth=request.getParameter("yr");
...
ResultSet rs=stmt.executeQuery("select  *  from health where regno= "+pd+"");
String sd;
if(rs.next())
{
   f=1;
   out.print("you have already submitted your health details");
...
if(f==0)
{
sd="insert into health values("+pd+",'"+reg+"','"+hos+"','"+locat+"','"+doc+"','"+stau+"','"+pla+"','"+auth+"')";

stmt.execute(sd);
out.print("successfully submitted  your health details");
   %><br>click here to <a href="NewApplication.jsp" target="right">continue</a><%
       }
   else
   {
   
         out.print("<br> your health details not submitted yet");
