<%-- 
    Document   : PropConnect.jsp
    Created on : 16 Feb, 2019, 11:25:22 PM
    Author     : Admin
--%>
 ...
 <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" ></script>  
    ...
    <ul style="list-style-type:none;  display:flex; justify-content: space-between; width:40%" >
       ...
       <a href="index.html"><i class="fa fa-plus-square"></i>
        ...
        <h1>PROPERTY CHECK PAGE!</h1>
          <h1>HEALTH VERIFICATION PAGE!</h1>
        <%
            int pk;
            int x;
        %>
...
Connection con  = DriverManager.getConnection("jdbc:mysql://localhost:3306/hsa","root","root");


 Statement stmt=con.createStatement();


ResultSet rs=stmt.executeQuery("select  *  from land where regno= "+pk);
String status,status1,status2;
 if(rs.next())
 {
    status=rs.getString(28);
    status1=rs.getString(29);
    status2=rs.getString(30);
    if(status.equals("DECLINE")|| status.equals("DECLINE") || status.equals("DECLINE"))
    {
        out.print("<br>Declined! Your application is rejected <br>");
    }
    else
    {
       if(status.equals("processing")|| status.equals("processing") || status.equals("processing"))
       {
           out.print("<br>! Your application is under consideration.Submit later <br>");
       }
       else
       {
           response.sendRedirect("PropF.jsp");
